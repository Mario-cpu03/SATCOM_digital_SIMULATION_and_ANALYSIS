...
fprintf('\n*************\n');
[BERNoCode, THROUGHPUTNoCode, PERNoCode, AWGNNoCode, ATMLOSSupNoCode, ATMLOSSdwNoCode, TEMPERATURENoCode, DENSITYNoCode, THERMALNOISEUPNoCode, THERMALNOISEDWNoCode, SNRNoCode] = NoChannelCod(MonteCarlo, NumMessages, BitTx, BitRx, BitAck);
...
fprintf('\n*************\n');
[BERCode, THROUGHPUTCode, PERCode, AWGNCode, ATMLOSSupCode, ATMLOSSdwCode, TEMPERATURECode, DENSITYCode, THERMALNOISEUPCode, THERMALNOISEDWCode,  SNRCode] = ChannelCod(MonteCarlo, NumMessages, BitTx, BitRx, BitAck);
...
PerformanceImprovGraph(mean(BERNoCode), mean(BERCode) ...
    ...
    , mean(AWGNNoCode), mean(AWGNCode) ...
    , mean(ATMLOSSupNoCode), mean(ATMLOSSupCode) ...
    , mean(ATMLOSSdwNoCode), mean(ATMLOSSdwCode) ...
    , mean(TEMPERATURENoCode), mean(TEMPERATURECode) ...
    , mean(DENSITYNoCode), mean(DENSITYCode) ...
    , mean(THERMALNOISEUPNoCode), mean(THERMALNOISEUPCode) ...
    , mean(THERMALNOISEDWNoCode), mean(THERMALNOISEDWCode) ...
    , mean(SNRNoCode), mean(SNRCode));
...
DataWriting(BERNoCode, BERCode ...
    ...
    , ATMLOSSdwNoCode, ATMLOSSdwCode ...
    , TEMPERATURENoCode, TEMPERATURECode ...
    , DENSITYNoCode, DENSITYCode ...
    , THERMALNOISEUPNoCode, THERMALNOISEUPCode ...
    , THERMALNOISEDWNoCode, THERMALNOISEDWCode ...
    , SNRNoCode, SNRCode);
